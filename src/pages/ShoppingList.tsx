...
export function ShoppingList() {
  const {
    selectedRecipes: contextRecipes,
    removeRecipe,
    clearList,
  } = useShoppingList();
  ...
  const handleCopy = () => {
    const text = aggregatedIngredients
      .map((i) => `- ${formatMetricAmount(i.amount, i.unit)} ${i.name}`)
      .join("\n");
    ...
    return (
      <div className="flex flex-col items-center justify-center min-h-[50vh] text-center space-y-4">
        ...
        <p className="text-gray-500 max-w-md">
          Go to the recipe library and click "Add to List" on recipes you want
          to cook.
        </p>
        <Link to="/">
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-1">
            Ingredients for {recipes.length} recipes:{" "}
          ...
          <div className="grid md:grid-cols-2 gap-x-12 gap-y-4">
            {aggregatedIngredients.map((ing, idx) => (
              ...
              >
                ...
                <span className="font-mono text-sm text-gray-600 font-medium whitespace-nowrap">
                  {formatMetricAmount(ing.amount, ing.unit)}
                </span>
              </div>
            ))}
          </div>
      ...
      <div className="bg-blue-50 p-4 rounded-lg text-sm text-blue-700 flex gap-2">
        ...
        <p>
          Note: This list combines ingredients by name and unit. Always check
